...
- **Branching:** work on a feature branch cut from `main`, rebase onto `main`, open a PR into `main`, and merge with *Rebase and merge*. No squash merges, no merge commits — history stays linear and every commit lands on `main` verbatim, which is why every commit must be conventional.
- **Releases:** fully automated on a **weekly cadence off `main`**. A GitHub Actions workflow (`.github/workflows/release.yml`, cron Monday 04:00 UTC + manual dispatch) checks out `main` and, when `cz bump --dry-run` finds releasable commits since the last tag, bumps the semver from the commit types, regenerates `CHANGELOG.md`, commits `chore(release): vX.Y.Z`, tags, pushes `main`, and publishes a GitHub Release. Merging a PR does not cut a release — the next Monday does; a week of only `chore`/`docs`/`style`/`test`/`build`/`ci` commits skips entirely. Never tag or edit `CHANGELOG.md` by hand. `modules/version.nix` surfaces the version as `system.nixos.label`. See [`.agents/skills/releasing`](.agents/skills/releasing/SKILL.md).
