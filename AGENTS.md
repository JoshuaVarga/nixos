...
- **Class/context mismatch silently drops content** — eval succeeds, but nothing applies. Content only takes effect when the resolving context has a forwarder for its class. The two cases we've hit:
  ...
  - `provides.<user>.<class>` routes **only when declared on a host aspect** — i.e. `den.aspects.<hostname>`. Mutual-provider keys on name matches, so `den.aspects.foo.provides.joshua.homeManager` on an arbitrary aspect `foo` included by titan is silently dropped. Either move the content to the host aspect's own `provides.<user>` block, or include the bare aspect from the user.
