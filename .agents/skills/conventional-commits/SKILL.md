...
- **Every commit ships.** PRs are *rebase*-merged into `main`, never squashed, so each
  commit lands on `main` verbatim and gets its own changelog line. There is no squash
  to hide a sloppy intermediate message behind — split work into atomic, individually
  meaningful commits.
- **`main` moves with every merged PR, plus the weekly `chore(release)` commit.** Always
  `git fetch origin && git rebase origin/main` before opening a PR; a release lands on
  `main` at most once a week and rewrites nothing, so rebasing is always clean.
